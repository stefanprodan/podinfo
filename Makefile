...
run:
	go run -ldflags "-s -w -X github.com/dee0sap/self-contained-podinfo/pkg/version.REVISION=$(GIT_COMMIT)" cmd/podinfo/* \
	--level=debug --grpc-port=9999 --backend-url=https://httpbin.org/status/401 --backend-url=https://httpbin.org/status/500 \
	--ui-logo=https://raw.githubusercontent.com/dee0sap/self-contained-podinfo/gh-pages/cuddle_clap.gif $(EXTRA_RUN_ARGS)
...
build:
	GIT_COMMIT=$$(git rev-list -1 HEAD) && CGO_ENABLED=0 go build  -ldflags "-s -w -X github.com/dee0sap/self-contained-podinfo/pkg/version.REVISION=$(GIT_COMMIT)" -a -o ./bin/podinfo ./cmd/podinfo/*
	GIT_COMMIT=$$(git rev-list -1 HEAD) && CGO_ENABLED=0 go build  -ldflags "-s -w -X github.com/dee0sap/self-contained-podinfo/pkg/version.REVISION=$(GIT_COMMIT)" -a -o ./bin/podcli ./cmd/podcli/*
...
.PHONY: cue-mod
cue-mod:
	@cd cue && go mod init github.com/dee0sap/self-contained-podinfo/cue
