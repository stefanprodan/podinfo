...
In order to trigger a release, please make sure to perform the following set of actions

```bash
# based on wether the release is a major, minor, or patch version we run on of the following commands in the terminal in the main branch
make release-major # or
make release-minor # or
make release-patch
```

This should update all references and commit the changes to upstream as well as push a tag.
We then go to the github repository and create the actual release.
